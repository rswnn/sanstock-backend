...
const insertMerchant = async (param) => {
  const { kode, nama, email, kontak, detail, created_at, updated_at} = param;
  const db = new Mysql(configs.get('/mysqlConfig'));
  const query = `INSERT INTO merchants (id, kode, nama, email, kontak, detail, created_at, updated_at) 
  VALUES (NULL, '${kode}', '${nama}', '${email}', '${kontak}', '${detail}', '${created_at}', '${updated_at}')`;
...
const updateMerchant = async (param) => {
  const { id, nama, email, kontak, detail, created_at, updated_at } = param;
  const db = new Mysql(configs.get('/mysqlConfig'));
  const query = `UPDATE merchants 
  SET nama = '${nama}', email = '${email}', kontak = '${kontak}', detail = '${detail}', updated_at = '${updated_at}'
